...
describe("api client", () => {
  ...
  it("builds the correct URL and attaches the bearer header", async () => {
    const calls = stubFetch({ data: [] });

    await api.listApps("tok_123");

    expect(calls).toHaveLength(1);
    expect(calls[0].url).toBe(`${API_BASE_URL}/v1/apps`);
  ...
  it("posts to the deploy action endpoint with the bearer token", async () => {
    const calls = stubFetch({ status: "queued" }, 202);

    const res = await api.deployApp("app_1", "tok_xyz");

    expect(calls[0].url).toBe(`${API_BASE_URL}/v1/apps/app_1/deploy`);
    expect(calls[0].init.method).toBe("POST");
    const headers = calls[0].init.headers as Record<string, string>;
